class Api::V1::RelationshipsController < ApplicationController
  ...
  def create
    ...
    others_relationship = Relationship.find_by(user_id: relationship.follow_id, follow_id: relationship.user_id)
    
    if relationship && !others_relationship
      room = Room.create
      #自分
      user_entry = Entry.find_or_create_by(user_id: relationship.user_id, room_id: room.id)
      #相手
      #relationship.user_id = others_relationship.follow_id 　#relationship.follow_id = others_relationship.user_id 
      other_user_entry = Entry.find_or_create_by(user_id: relationship.follow_id, room_id: room.id) 
